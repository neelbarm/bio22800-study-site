...
<ul>
<li><b>Routine</b> — on a documented schedule. AIUM accreditation standards call for at least annual QA; local policy, manufacturer guidance and higher-risk checks may be more frequent, with checks after repair or transducer replacement.</li>
...
<text x="112" y="62" fill="currentColor" font-size="15">early values pass</text>
<text x="300" y="252" fill="var(--c4)" font-size="15">trend warns before the final failure</text>
</svg>`,
...
<p>The liquid is a water/alcohol mixture tuned to 1,540 m/s. Nylon filaments around 0.1 mm are small compared with the resolution being tested, so they approximate point targets; at very high frequencies they are not necessarily much smaller than one wavelength.</p>
...
<text x="34" y="340" fill="currentColor" font-size="15">water + alcohol, tuned to 1,540 m/s</text>
...

    {
      ...
      html: `<p class="lead">Two geometry tests catch different faults: how close to the face you can see, and whether independent views agree in a known registered coordinate system.</p>
...
<p><b>Registration accuracy</b> uses fixed, known test geometry to ask whether views agree after their positions are mapped into a common coordinate system. Translating a freehand probe normally changes screen coordinates, so “same screen spot from any window” is not the rule.</p>
...
<p>The <b>reference standard</b> is the best available comparator — surgery, biopsy, angiography or follow-up. It defines truth for the study, but it can itself have sampling or classification error.</p>
...
<p>Start with everyone classified healthy by the reference standard: what fraction were correctly called negative? That is specificity. The exam heuristic is <b>SnNout, SpPin</b>: a highly sensitive test's negative result helps rule out; a highly specific test's positive result helps rule in. Neither creates certainty by itself.</p>
<div class="steps"><ol><li>TP=45, FN=5: sensitivity=45/50=90%.</li><li>TN=40, FP=10: specificity=40/50=80%.</li><li>Accuracy=(45+40)/100=85%; PPV=45/55=81.8%; NPV=40/45=88.9%.</li><li>Prevalence=(45+5)/100=50%; FN rate=10%; FP rate=20%.</li></ol></div>
...
<text x="468" y="200" fill="var(--c3)" font-size="15">NPV down</text>
  ...
  keyNumbers: [
    ...
    { fact: "Nylon pin diameter in a test object", value: "≈ 0.1 mm — small relative to the tested resolution" },
    ...
    { fact: "QA testing interval", value: "At least annual for AIUM accreditation; local/manufacturer policy may require more often and after repair" },
  ...
  questions: [
    ...
    { id: "u15-q3", type: "short", q: "What QA feature lets a department detect gradual scanner drift over time?", answer: "consistent repeated testing", accept:["consistent repeated testing","repeated testing","periodic testing","consistent testing","trend testing"], explain:"Comparable tests at scheduled intervals reveal trends. AIUM accreditation calls for documented QA at least annually; local policy, manufacturer guidance and post-repair checks may be more frequent.", objectives:["u15-o1"],lesson:"u15-l1",level:1 },
    ...
    { id: "u15-q9", type: "mc", q: "Why are test-object filaments made very small relative to the resolution being tested?", choices:["To approximate point targets","To attenuate like tissue","To resonate with the probe","To eliminate the dead zone"],answer:0,explain:"A small target lets the displayed spot reflect system resolution rather than target width. A 0.1 mm filament is not below every diagnostic wavelength, so the comparison must be to the tested resolution.",objectives:["u15-o3"],lesson:"u15-l3",level:2 },
    ...
    { id: "u15-q18", type: "tf", q: "In a fixed registered test geometry, independent views of a known target should agree in the common spatial coordinate system.", answer:true,explain:"Registration compares known geometry after accounting for probe/view position. Simply translating a probe should move the target's screen coordinate, so 'same screen spot from any window' is misleading.",objectives:["u15-o5"],lesson:"u15-l4",level:1 },
    ...
    { id: "u15-q26", type: "mc", q: "Which classification means disease is present but the test misses it?", choices:["True positive","False positive","True negative","False negative"],answer:3,explain:"That is a false negative. Clinical harm depends on context; false positives can also cause substantial harm, so there is no universal risk ranking.",objectives:["u15-o7"],lesson:"u15-l7",level:2 },
    ...
    { id: "u15-q35", type: "short", q: "At fixed sensitivity and specificity, which two predictive statistics change directly with prevalence?", answer:"PPV and NPV",accept:["ppv and npv","predictive values","positive and negative predictive value","ppv npv","ppv, npv"],explain:"PPV and NPV change directly with prevalence. Overall accuracy can also change as the case mix changes; observed sensitivity/specificity may vary through spectrum effects.",objectives:["u15-o9"],lesson:"u15-l8",level:2 },
  ...
  drills: [
    ...
    { id:"u15-d7",title:"Prevalence, error rates and calibration",formula:"prevalence; FN/FP rates; range; measurement error",lesson:"u15-l8",gen:function(rnd){var mode=Math.floor(rnd()*4);if(mode===0){var tp=30+Math.floor(rnd()*40),fn=5+Math.floor(rnd()*15),total=100+Math.floor(rnd()*100),v=(tp+fn)/total*100;return {kind:"number",given:"TP="+tp+", FN="+fn+", total="+total,ask:"Prevalence?",answer:+v.toFixed(2),unit:"%",tol:0.2,steps:["prevalence=(TP+FN)/total×100","=("+tp+"+"+fn+")/"+total+"×100","="+v.toFixed(2)+"%"]};}if(mode===1){var a=5+Math.floor(rnd()*15),b=40+Math.floor(rnd()*50),v=a/(a+b)*100;return {kind:"number",given:"FN="+a+", TP="+b,ask:"False-negative rate?",answer:+v.toFixed(2),unit:"%",tol:0.2,steps:["FN rate=FN/(TP+FN)×100","="+a+"/("+b+"+"+a+")×100","="+v.toFixed(2)+"%"]};}if(mode===2){var t=[52,78,130][Math.floor(rnd()*3)],d=1.54*t/2;return {kind:"number",given:"Phantom echo round-trip time="+t+" µs",ask:"Depth?",answer:+d.toFixed(2),unit:"mm",tol:0.2,steps:["depth=1.54×time/2","=1.54×"+t+"/2","="+d.toFixed(2)+" mm"]};}var truth=[10,20,50][Math.floor(rnd()*3)],meas=truth+[0.2,0.5,1][Math.floor(rnd()*3)],e=(meas-truth)/truth*100;return {kind:"number",given:"True="+truth+" mm, measured="+meas+" mm",ask:"Signed measurement error?",answer:+e.toFixed(2),unit:"%",tol:0.2,steps:["error=(measured−true)/true×100","=("+meas+"−"+truth+")/"+truth+"×100","="+e.toFixed(2)+"%"]};} }
  ],
